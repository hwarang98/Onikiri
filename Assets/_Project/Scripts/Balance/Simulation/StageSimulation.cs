...
namespace Onikiri.Progression
{
    ...
    public static class StageSimulation
    {
        ...
        public struct Policy
        {
            ...
            /**
             * @brief **귀문을 한 번도 깨지 않는다.** 승급 재설계 2단계에 뜻이 바뀌었다.
             *
             * 33단계에는 "전직을 안 산다"였다 - 재화를 쓸지 말지의 선택이었다.
             * 승급이 귀문 돌파로 무료가 되면서 그 선택이 사라졌고, 이제 이
             * 정책이 재는 것은 **"문을 넘지 못한 플레이어의 세계"**다.
             *
             * 3단계에서 진행이 실제로 막히면 이 세계는 게이트 스테이지 이후로
             * 존재하지 않게 된다 - 그때 이 정책은 "귀문이 없었다면 어땠는가"를
             * 재는 반사실 비교군으로만 남는다.
             */
            public bool SkipEvolution;

            /**
             * @brief 전직 배수가 없는 세계. **이제 SkipEvolution과 같다.**
             *
             * 33~1.6단계에는 둘이 갈렸다 - 저쪽은 배수만, 이쪽은 배수와
             * 보정(StageCurve.EvolutionCompensation)을 함께 걷었다. 2단계에
             * 보정항 자체가 사라지면서 걷어낼 두 번째 것이 없어졌다.
             *
             * 이름을 남겨 둔 이유는 `PromotionBandModel`의 오버레이가 이
             * 정책의 런을 **중립 세계**로 쓰기 때문이다 - "배수도 보정도 없는
             * 바탕"이라는 뜻은 그대로이고, 그 위에 곱셈 한 번으로 어떤 지수의
             * 세계든 재구성된다.
             */
        ...
        public static List<StageResult> Run(int throughStage, Field field, Policy policy)
        {
            ...
            for (int stage = 1; stage <= throughStage; stage++)
            {
                ...
                // "배수도 보정도 없는 바탕"이어야 `PromotionBandModel`의 오버레이가
                // 이중으로 곱하지 않는다. 티어 4까지는 1이라 st1~100은 이 줄에
                // 반응하지 않는다
                if (policy.NeutralizeEvolution) bossKill /= StageCurve.DeepPromotionCompensation(stage);
                                ...
                                int currentStage, Policy policy, ref double purchasePayback)
        {
            ...
            // 전직은 이제 **저울 밖이다.** 승급 재설계 2단계에 재화가 빠지면서
            // 살 것도 미룰 것도 없어졌고, 남은 것은 "지나온 문의 수를 티어에
            // 반영한다" 한 줄이다.
            //
            // 자리를 여기 그대로 둔 이유는 순서가 여전히 뜻을 갖기 때문이다 -
            // 티어가 오르면 공격·체력 배수가 함께 오르고, 아래 화력 루프가
            // 그 배수 위에서 효율을 잰다. 뒤로 옮기면 그 스테이지의 구매가
            // 한 칸 옛 배수로 계산된다.
            //
            // (옛 판은 골드 도약 비용이 지갑을 비워 생존 축을 굶기는 문제가
            // 있었고, 그래서 생존 다음 자리가 불변식이었다. 비용이 0이 되어
            // 그 위험 자체가 사라졌다.)
            TryEvolve(ref levels, policy, currentStage);
        ...
        /**
         * @brief 전직 티어를 **지나온 문의 수**에 맞춘다. 승급 재설계 2단계.
         *
         * ## 재화도 레벨도 보지 않는다
         *
         * 옛 판은 넷을 봤다 - 해금 레벨(Lv.30), 보석, 골드, 상한. 승급이 귀문
         * 돌파로 무료가 되면서 앞의 셋이 통째로 사라졌다. 남은 것은 **최전선이
         * 문을 몇 개 지났는가** 하나이고, 그것은 스테이지의 함수이므로 루프도
         * 필요 없다.
         *
         * 이 함수가 이제 재화를 안 건드린다는 사실이 두 곳에 나타난다 -
         * `GemsSpent`가 안 오르고 `purse`가 안 준다. 그래서 보석 하한
         * 플레이어의 지갑이 장비·동료로 통째로 간다.
         *
         * ## 비교군의 뜻이 바뀌었다
         *
         *   SkipEvolution        "전직을 안 산다" -> **"귀문을 한 번도 안 깬다"**
         *   NeutralizeEvolution  보정항이 사라졌으므로 SkipEvolution과 같은 세계다.
         *                        `PromotionBandModel`의 중립 런이 이 정책을 쓴다
         *
         * @param frontier 지금 서 있는 스테이지. 문은 `gateStage < frontier`일 때 지났다
         */
        private static void TryEvolve(ref Levels levels, Policy policy, int frontier)
        {
            if (policy.SkipEvolution || policy.NeutralizeEvolution) return;

            int tier = PromotionTrialCatalog.TierAtFrontier(frontier);
            if (tier > EvolutionCurve.MaxTier) tier = EvolutionCurve.MaxTier;

            // 내려가지 않는다. 재선택으로 뒤 스테이지에 서도 이미 넘은 문은
            // 그대로다 - 마이그레이션의 `max`와 같은 규칙이다
            if (tier > levels.Ev) levels.EvolutionTier = tier;
        }
