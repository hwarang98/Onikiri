...
namespace Onikiri.Progression
{
    ...
    public static class BossCurve
    {
         ...
         *
         * 26단계에 1.25 -> 1.18로 내렸다. 예고한 대로 새 축(스킬) 때문에 다시
         * 왔고, 이번 이유는 축 자체가 아니라 **축을 상쇄하려고 올린 램프**다.
         * 램프는 등급을 가리지 않아서 이미 바닥 쪽에 있던 챕터를 함께 눌렀고
         * (st5가 1.32로 바닥 1.30에 붙었다), 램프가 내린 만큼 등급 배수로
         * 되올린다. FinaleHealthMultiplier와 같은 처리이고 이유도 같다 -
         * **램프는 기울기, 등급 배수는 간격**이다.
         *
         * 최종 챕터 여유는 1.37~1.71로 밴드[1.3~2.0] 안이다.
         */
        public const double ChapterHealthMultiplier = 1.18d;
         ...
         *
         * ## 26단계에 1.7에서 1.38로 되돌렸다
         *
         * 이 주석이 예고한 일이 그대로 다시 일어났다. 스킬 축이 들어오면서
         * 램프를 두 끝 모두 올려야 했는데(Final 1.110 -> 1.130, Start 1.34 -> 1.36),
         * **램프는 등급을 가리지 않으므로** 이미 밴드 바닥 근처였던 피날레가
         * 함께 눌렸다. 1.7을 유지하면 st20 피날레가 1.16으로 바닥(1.15)에
         * 0.01까지 붙는다 - 16단계가 남긴 "얇은 경계"를 갚으러 온 스텝에서
         * 같은 종류의 부채를 새로 지는 셈이다.
         *
         * 램프가 등급 전체를 내리는 만큼 등급 배수로 되올린다. 두 손잡이의
         * 역할이 그래서 갈린다 - **램프는 기울기, 등급 배수는 등급 사이의
         * 간격**이다. 기울기를 고치느라 간격이 함께 움직였으면 간격은 따로
         * 되돌려야 한다.
         *
         * 1.55 -> 1.45 -> 1.38로 세 번 재며 내렸다. 램프를 한 칸 올릴 때마다
         * 여기를 다시 봐야 했다는 뜻이고, 그것이 이 계수의 성질이다. 최종
         * 피날레 여유는 1.22~1.61로 밴드[1.15~1.7] 한가운데에 선다.
         */
        /**
         * ## 32단계에 1.38 -> 1.46. 예고대로 또 왔고, 이번에는 방향이 반대다
         *
         * 26단계까지 이 값은 램프에 눌린 피날레를 **되올리는** 쪽으로 움직였다.
         * 이번에는 내린다 - 장비가 피날레만 골라 밀어 올렸기 때문이다.
         *
         * 이유가 구조에 있다. 장비는 st11에 열리므로 **st10 이전의 등급에는
         * 아무 일도 하지 않는다.** 그런데 챕터의 최악 여유는 st5(1.41)이고
         * 피날레의 최악은 st20이다. 장비가 st20을 1.35에서 1.43으로 올리자
         * 피날레 최악이 챕터 최악을 넘었고, 그 순간 "지역의 마지막이 가장
         * 빡빡하다"가 수치에서 거짓이 된다
         * (StageSimulationTests.ChapterBosses_AreTighterThanStageBosses가 잡았다).
         *
         * 램프로는 고칠 수 없다. 램프는 등급을 가리지 않으므로 st5도 함께
         * 내려가 챕터 바닥(1.30)을 뚫는다. **기울기가 아니라 간격이 어긋난
