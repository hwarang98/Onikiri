...
namespace Onikiri.Progression
{
    ...
    public static class GoldGainCurve
    {
        ...
        /**
         * @brief 해금된 뒤 상한까지 오르는 데 걸리는 스테이지 수. **0 = 즉시.**
         *
         * 0인 이유는 실측이다. 해금(6스테이지) 시점에는 수입이 이미 커서 회수
         * 시간이 밴드 한참 아래이고, 곡선 추종 플레이어는 **그 스테이지 안에
         * 상한까지 사버린다**(시뮬레이션 gLv: st5까지 1, st6에 13 = 상한).
         *
         * 20단계에는 해금이 없어서 축이 1~5에 걸쳐 천천히 자랐고, 그래서 과도기를
         * 지수로 보간했다. 게이트가 생기면서 그 과도기 자체가 사라졌다.
         *
         * 값이 다시 0보다 커지면(비용을 올리거나 상한을 키우면) 보간이 되살아난다.
         * `GoldAxis_ReachesCeilingWhenExpected`가 실측과 대조한다 - 상수의 존재는
         * 연결의 증거가 아니다.
         */
