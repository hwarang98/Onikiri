...
namespace Onikiri.Battle
{
    ...
    public sealed class PlayerCombat : MonoBehaviour
    {
        ...
        {
            return DeliverSkillLane(originX, originY, range, height, multiplier, tint,
                numberSizeMultiple, Onikiri.Progression.TrialDamageScale.Source.Skill);
        }

        /** 출처를 밝힌 관통. 영체의 휩쓸기가 이쪽을 쓴다 (`DeliverSkillHit` 주석 참고) */
        public int DeliverSkillLane(float originX, float originY, float range, float height,
                                    BigDouble multiplier, Color tint, int numberSizeMultiple,
                                    Onikiri.Progression.TrialDamageScale.Source skillDamageSource)
        {
            ...
            for (int i = 0; i < enemies.Count; i++)
            {
                ...
                if (DeliverSkillHit(enemy, multiplier, tint, numberSizeMultiple, skillDamageSource)) hits++;
            }
            return hits;
        }
        ...
        /**
         * @brief 지금 이 순간의 **귀문 점수 여섯 항.** 소프트캡이 입장 시 한 번 읽는다.
         *
         * ## 왜 PlayerCombat이 내는가
         *
         * 여섯 항의 재료(공격력·공격속도·치명타·초월·연격·시전율)가 전부 여기
         * 사적 필드에 있다. 밖에서 읽으려면 그것들을 공개해야 하고, 그러면
         * 두 번째 계산기가 생겨 `ExpectedDps`와 갈릴 자리가 만들어진다 -
         * 이 프로젝트가 반복해서 물린 종류의 결함이다.
         *
         * ## 항의 분해 - 무엇을 어디에 넣었는가
         *
         * ```
         * base       = 공격력 x 치명타 기대배수 x 초월
         * combo      = (1 + 연격 확률)          <- 모든 타격에 걸리므로 세 항에 공통
         *
         * AutoAttack = base x 공격속도       x combo
         * Skills     = base x 오의 시전율     x combo
         * Specials   = base x 영체 시전율     x combo
         * MultiHit   = 0                      <- 연격이 위 셋에 **이미** 곱해져 있다
         * DoT        = 0                      <- 지속 피해 시스템이 아직 없다
         * Companions = (위 셋의 합) x 동료 보너스 합
         * ```
         *
         * `MultiHit`을 0으로 두는 것이 중요하다. 연격을 따로 세면 같은 타격이
         * 두 칸에 들어가고, 그러면 점수가 부풀어 캡이 실제보다 세게 걸린다 -
         * 그 플레이어만 손해를 본다.
         *
         * `DamageOverTime`도 0이다. 지어낸 값을 넣지 않는다 - 지속 피해가
         * 생기는 날 이 줄이 그 자리를 가리킨다.
         *
         * ## 합이 `ExpectedDps`와 같아야 한다
         *
         * 동료를 뺀 다섯 항의 합이 `ExpectedDps`와 정확히 같다. 두 식이 갈리면
         * 점수와 방치 보상이 다른 플레이어를 재게 되고,
         * `PromotionTrialDamageTests.TrialPowerMatchesExpectedDps`가 그것을 잡는다.
         */
        public Onikiri.Progression.TrialPowerScore.Components ReadTrialPower()
        {
            float factor = 1f + critChance * (critMultiplier - 1f);
            float combo = 1f + comboChance;

            var unit = damage * BigDouble.FromDouble(factor * transcendMultiplier * combo);

            double auto = (unit * BigDouble.FromDouble(attacksPerSecond)).ToDouble();
            double skills = (unit * BigDouble.FromDouble(
                (float)Onikiri.Progression.SkillSystem.CurrentCastRate)).ToDouble();
            double specials = (unit * BigDouble.FromDouble(
                (float)Onikiri.Progression.YodoSystem.CurrentSpiritRate)).ToDouble();

            // 동료는 플레이어 DPS의 비율로 들어온다(PetCombat.HitDamage). 보너스가
            // 플레이어 DPS **전체**에 걸리므로 세 항의 합에 곱한다
            double companionBonus = 0d;
            var pets = Onikiri.Progression.PetSystem.Instance;
            if (pets != null)
                for (int i = 0; i < pets.PetCount; i++) companionBonus += pets.FieldedBonusOf(i);

            return new Onikiri.Progression.TrialPowerScore.Components
            {
                AutoAttack = auto,
                Skills = skills,
                MultiHit = 0d,
                DamageOverTime = 0d,
                Companions = (auto + skills + specials) * companionBonus,
                BossApplicableSpecials = specials
            };
        }
        ...
        private void DeliverHit(Enemy target)
        {
            ...
            // **화면에 뜨는 숫자는 실제로 깎인 양이다.** 귀문의 공통 배율이
            // 여기서 걸리므로 raw를 표시하면 숫자와 체력이 어긋난다. 배율을
            // 여기서 다시 곱하지 않는 이유는 그러면 곱셈이 두 곳이 되고,
            // 둘이 갈리는 날 이중 적용이 되기 때문이다
            dealt = target.TakeDamage(dealt, Onikiri.Progression.TrialDamageScale.Source.AutoAttack);
            ...
            {
                SpawnSpark(target, hitPoint);
                // 연격은 평타의 **한 대 더**다. 출처를 따로 두지 않는 이유는
                // 점수 쪽에서도 연격이 평타 항에 곱해져 있기 때문이다 -
                // 나누면 감사 표와 점수 표가 다른 칸을 세게 된다
                var comboDealt = target.TakeDamage(dealt, Onikiri.Progression.TrialDamageScale.Source.AutoAttack);

                if (damageNumbers != null)
                    damageNumbers.Show(comboDealt, hitPoint,
        ...
        {
            return DeliverSkillHit(target, multiplier, tint, numberSizeMultiple,
                Onikiri.Progression.TrialDamageScale.Source.Skill);
        }

        /**
         * @brief 출처를 밝힌 오의 타격. **영체 소환이 이쪽을 쓴다.**
         *
         * 오의와 영체가 같은 함수를 지나면서 감사 표에서는 갈려야 한다 -
         * `TrialPowerScore`가 둘을 다른 항(`Skills` / `BossApplicableSpecials`)으로
         * 세기 때문이다. 앰비언트 상태 대신 인자로 받는 이유는 그것을 되돌리는
         * 것을 잊는 경로가 생기면 영체가 오의로 집계되고, 그 오류가 화면에
         * 안 뜨기 때문이다.
         */
        public bool DeliverSkillHit(Enemy target, BigDouble multiplier, Color tint,
                                    int numberSizeMultiple,
                                    Onikiri.Progression.TrialDamageScale.Source skillDamageSource)
        {
            ...
            dealt = target.TakeDamage(dealt, skillDamageSource);
            bool killed = !target.IsAlive;
            ...
            {
                SpawnSpark(target, hitPoint);
                var comboDealt = target.TakeDamage(dealt, skillDamageSource);

                if (damageNumbers != null)
                    damageNumbers.ShowSkill(comboDealt, hitPoint, tint, numberSizeMultiple);
         ...
         * 않는 이유는 시전 빈도다 - 리듬 재조정(쿨다운 2.5~11초) 뒤에도 기준
         * 구성의 시전은 초당 0.7회 안팎이라, 시전당 0.15초를 멈춰도 초당
         * 0.1초 - 평타 예산 0.3초/초의 1/3이다. 같은 잣대로 재면 오의는
         * 여전히 예산 안에 있다.
         *
