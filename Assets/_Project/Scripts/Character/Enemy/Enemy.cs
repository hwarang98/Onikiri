...
namespace Onikiri.Battle
{
    ...
    public sealed class Enemy : MonoBehaviour
    {
        ...
        /**
         * @brief 귀문(승급전)의 적인가. **보상 경로 전체를 막는 플래그다.**
         *
         * 귀문의 3체는 골드·경험치·보석·퀘스트·요도·클리어 보너스를 **어떤
         * 경로로도** 주지 않는다. 그 문을 여섯 번 넘는 것이 진행의 조건인데
         * 거기서 보상이 나오면 재도전이 파밍이 되고, 무료·무제한 재도전이
         * 무한 수입원이 된다.
         *
         * 보상을 0으로 스폰하는 것만으로는 모자라다 - `EnemySpawner`가
         * `IsBoss`만 보고 `QuestSystem.ReportBossKill()`을 부르므로, 값이 0이어도
         * 퀘스트 카운터는 오른다. 그래서 값이 아니라 **플래그**가 필요하다.
         *
         * `Killed`/`EnemyKilled` 사건 자체는 막지 않는다 - 상태 머신이 그것으로
         * 다음 적을 세운다.
         */
                          ...
                          float scale = 1f, Color tint = default(Color), double attackDamage = 0d)
        {
            ...
            // 귀문 설정은 스폰마다 **반드시 꺼진다.** 풀에서 돌아온 인스턴스가
            // 옛 판의 간격과 보상 차단을 들고 있으면, 일반 스테이지의 잡몹이
        ...
        /**
         * @brief 피해를 받는다. **플레이어 측 피해가 요괴 체력에 닿는 유일한 경로다.**
         *
         * 출처를 안 밝히는 옛 호출을 그대로 받는다. 새 인자를 필수로 만들면
         * 배선을 잊은 곳이 컴파일 오류로 드러나 좋을 것 같지만, 실제로는
         * 테스트 씬과 도구가 전부 멈춘다 - 감사가 필요한 것은 **귀문 안**이고
         * 그 안에서 태그 없는 피해가 0인지는 검사가 따로 잰다
         * (`PromotionTrialDamageTests.EveryDamageSourceIsAttributed`).
         */
        public BigDouble TakeDamage(BigDouble amount)
        {
            return TakeDamage(amount, Onikiri.Progression.TrialDamageScale.Source.Unattributed);
        }

        /**
         * @brief 출처를 밝힌 피해. **귀문의 공통 배율이 여기서 한 번 걸린다.**
         *
         * ## 왜 여기인가
         *
         * `health -= amount`가 이 프로젝트 전체에서 한 줄이다. 소프트캡을 그
         * 바로 앞에 두면 평타·오의·다단·연격·동료·영체가 전부 같은 배율을
         * **한 번씩** 지나고, 새 피해 출처가 생겨도 배선을 잊을 자리가 없다.
         *
         * 캡을 각 호출자에 두는 쪽을 안 고른 이유가 그것이다 - 다섯 곳에 흩으면
         * 여섯 번째가 생기는 날 그것만 캡 밖에 서고, 그 결함은 화면에 안 뜬다.
         *
         * ## 무엇이 여기 없는가
         *
         * 적이 플레이어에게 주는 피해는 `PlayerHealth.TakeDamage`이고 이 함수를
         * 지나지 않는다. 캡은 플레이어의 과잉 화력만 완만하게 한다.
         */
        /**
         * @return **실제로 체력에서 깎인 양.** 부르는 쪽이 데미지 숫자에 쓴다.
         *
         * 반환값이 있는 이유는 귀문에서 화면과 체력이 어긋나는 것을 막기
         * 위해서다. 호출자가 raw를 넘기고 raw를 표시하면, 캡이 걸린 문 안에서
         * 숫자는 100을 띄우는데 체력은 60만 줄어든다 - 플레이어가 읽을 수 있는
         * 정보가 거짓이 된다.
         *
         * 호출자가 배율을 **다시 곱해** 표시하는 방법도 있지만 그러면 곱셈이
         * 두 곳이 되고, 그 둘이 갈리는 날 이중 적용이 된다. 결과를 한 곳에서
         * 내려보내는 것이 이 반환값의 전부다.
         *
         * 이미 죽은 요괴는 0을 낸다 - 아무것도 안 깎였으므로 숫자도 안 떠야 한다.
         */
        public BigDouble TakeDamage(BigDouble amount, Onikiri.Progression.TrialDamageScale.Source source)
        {
            if (!IsAlive) return BigDouble.Zero;

            amount = Onikiri.Progression.TrialDamageScale.Apply(amount, source);
         ...
         */
        /**
         * @brief 귀문에서 덮어쓴 공격 간격(초). 0이면 정의 애셋의 값을 쓴다.
         *
         * 일반 적과 일반 보스는 이 값이 0이라 이 스텝 이전과 **완전히 같은
         * 경로**를 지난다 - 귀문 전용 설정이 다른 전투로 새어 나가지 않는다는
         * 것이 `ConfigureAsTrialFoe`의 계약이고, 스폰마다 0으로 되돌아간다.
         */
        private float trialAttackInterval;

        /**
         * @brief 이 요괴를 **귀문의 적**으로 설정한다. 스폰 직후 한 번 부른다.
         *
         * ## 왜 정의 애셋의 간격을 안 쓰는가
         *
         * 계산기가 45초 앵커를 유도할 때 쓴 값이 **간격 2초 · 첫 타격 2초**다
         * (`PromotionTrialCatalog`). 적 정의의 `attackInterval`은 그 요괴의
         * 연출 리듬이라 문마다 다르고, 그 값을 그대로 쓰면 실기가 계산기와
         * 갈린다 - 재생 임계(보스 DPS / 초당 재생 = 0.88~1.22)가 칼날이라
         * 간격 몇 십분의 일 초가 생사를 가른다.
         *
         * 그래서 귀문에서만 덮어쓴다. 일반 보스는 이 함수를 지나지 않는다.
         *
         * @param interval    공격 간격(초). `PromotionTrialCatalog.FoeAttackIntervalSeconds`
         * @param firstDelay  첫 타격까지의 지연(초). 계산기의 `FirstAttackDelaySeconds`
         */
        public void ConfigureAsTrialFoe(float interval, float firstDelay)
        {
            ...
            // 뒤에서 시작하는 것이 곧 "등장 후 firstDelay 뒤 첫 타격"이다
            attackTimer = interval > 0f ? interval - firstDelay : 0f;
        ...
        /**
         * @brief 공격력을 갈아 끼운다. **귀문의 격노만 쓴다.**
         *
         * 90초 뒤 10초마다 x1.3이 누적되므로 스폰 시점의 값으로는 표현할 수
         * 없다. 일반 보스는 이 함수를 안 부르므로 스폰 값이 끝까지 간다.
         */
        ...
        {
            if (AttackDamage <= 0d || definition == null) return;
            if (CurrentState != State.Engaged) return;

            // 귀문이면 덮어쓴 간격, 아니면 정의 애셋의 값. 둘 다 0이면 안 때린다
            // (잡몹이 공격하지 않는 것은 설계다 - PlayerHealth 참고)
            float interval = trialAttackInterval > 0f ? trialAttackInterval : definition.attackInterval;
