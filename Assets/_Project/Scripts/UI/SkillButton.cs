...
namespace Onikiri.UI
{
     ...
     * ## 쿨다운을 아랫줄에 함께 적는 이유
     *
     * 이 축의 값어치는 배율 하나로 읽히지 않는다. 귀참 x3.52와 연참 x0.54를
     * 나란히 놓으면 귀참이 여섯 배 좋아 보이지만 실제 기여는 쿨다운으로 나눈
     * 값이고(0.32 대 0.18) 두 배도 차이가 나지 않는다. 배율만 보여주면 화면이
     * 거짓말을 한다.
     *
     * 그래서 아랫줄이 `x0.54 -> x0.60  ·  3.0초`다. 초당 환산값까지 적지는
    ...
    public sealed class SkillButton : MonoBehaviour
    {
        ...
        /**
         * @brief 줄을 누르면 **팝업이 열린다.** 바로 강화하지 않는다 (#14).
         *
         * 26단계부터 이 줄은 곧 구매 버튼이었다(강화 목록과 같은 문법).
         * 오의에서 그 문법이 안 맞는 이유는 이름만으로 무엇을 하는지 알 수
         * 없기 때문이다 - "혈파동"이 한 대상을 때리는지 화면을 쓸어버리는지가
         * 화면 어디에도 없었다(SkillInfoPopup 머리 주석).
         *
         * 강화는 그 팝업 안에서 같은 함수를 부른다. 여기서 부르던 것과
         * 완전히 같은 경로라, 옮겨간 것은 **누르는 자리**뿐이다.
         *
         * 팝업이 배선되지 않은 씬(전투 전용 테스트)에서는 예전처럼 곧바로
         * 산다 - 화면 하나가 없다고 강화 자체가 막히지는 않는다.
         */
        private void OnClick()
        {
            if (popup != null) { popup.Open(slotIndex); return; }
            if (system != null) system.TryPurchase(slotIndex);
        }

        [Tooltip("이 줄이 여는 정보 팝업 (#14). 비면 예전처럼 곧바로 강화한다")]
        [SerializeField] private SkillInfoPopup popup;
