...
namespace Onikiri.Battle
{
    ...
    public sealed class EnemySpawner : MonoBehaviour
    {
        ...
        private void OnEnemyKilled(Enemy enemy)
        {
            ...
            /**
             * @brief **귀문의 적은 보상 경로를 하나도 지나지 않는다.**
             *
             * 골드·경험치·스테이지 할당량·퀘스트(잡몹/보스) 전부 아래에서
             * 갈라진다. 요도 판정과 클리어 보너스는 `BossFight`가 자기 보스에만
             * 주므로 여기 없다.
             *
             * 보상을 0으로 스폰하는 것만으로는 모자란 이유가 이 함수에 있다 -
             * 퀘스트는 값이 아니라 `IsBoss`만 보고 카운터를 올린다. 귀문이
             * 무료·무제한 재도전인데 그 카운터가 오르면 재도전이 퀘스트 파밍이
             * 된다.
             *
             * `EnemyKilled`는 **막지 않는다.** 상태 머신이 그것으로 다음 적을
             * 세우고 승리를 판정한다 - 사건과 보상을 가르는 것이 이 분기의 전부다.
             */
            if (enemy.IsTrialFoe)
            {
                var trialHandler = EnemyKilled;
                if (trialHandler != null) trialHandler(enemy);
                return;
            }
