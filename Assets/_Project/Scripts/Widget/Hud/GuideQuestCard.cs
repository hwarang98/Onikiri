...
namespace Onikiri.UI
{
    ...
    public sealed class GuideQuestCard : MonoBehaviour
    {
        ...
        /**
         * @brief 귀문 진입 (4단계 §3). **대기 중에는 이 카드가 그 입구다.**
         *
         * 게이트 보스를 벤 순간 진행이 막히는데, 그때 화면이 말해야 하는 것은
         * "요괴 500마리 처치 4/100"이 아니라 "일문 도전"이다. 다음에 할 일을
         * 한 장으로 말하는 것이 이 카드의 존재 이유이고, 귀문 대기는 그 말이
         * 가장 분명한 순간이다.
         *
         * 입구를 **여기 하나로** 모은다. 하단 보스 도전 버튼이 「귀문 도전」으로
         * 갈리던 것을 걷어냈다(BossHud) - 같은 것을 여는 버튼이 화면에 둘 있으면
         * 그중 하나는 반드시 안 눌리는 장식이 된다.
         */
        [Header("귀문 (4단계 §3)")]
        [SerializeField] private Onikiri.Battle.BossFight fight;

        [Tooltip("카드 전체의 버튼. 평소엔 HudScreenButton이, 귀문 대기 중엔 이쪽이 쓴다")]
        [SerializeField] private Button cardButton;

        [Tooltip("카드 전체의 퀘스트 화면 열기. 귀문 대기 중에는 눌림을 막는다")]
        [SerializeField] private HudScreenButton cardScreenButton;
        ...
        private void OnEnable()
        {
            ...
            if (fight != null) fight.Changed += OnChanged;
        ...
        private void OnDisable()
        {
            ...
            // 카드가 꺼진 채로 억제만 남으면 퀘스트 화면이 영영 안 열린다
            if (cardScreenButton != null) cardScreenButton.Suppressed = false;
        ...
        /** 귀문이 열려 대기 중인가. 이 상태에서 카드는 문 입구가 된다 */
        private bool TrialPending
        {
            get { return fight != null && fight.CanChallengeTrial; }
        }

        /** 지금 카드가 귀문 안내를 그리고 있는가 */
        private bool trialShown;

        /**
         * @brief 귀문 대기면 그쪽을 그리고 true. 아니면 흔적을 지우고 false.
         *
         * 갱신 경로 둘(OnChanged · Refresh)이 **같은 문을 지나야** 한 쪽만
         * 귀문을 아는 상태가 안 생긴다.
         */
        private bool HandleTrialMode()
        {
            if (TrialPending) { PaintTrial(); return true; }
            ...
            if (trialShown)
            {
                ...
                // 다음 그리기를 강제한다. 안 비우면 귀문 전과 같은 칸·상태일 때
                // "달라진 것이 없다"로 판정돼 카드가 귀문 문구인 채로 남는다
                shown = default(GuideQuestView);
                shown.Step = -1;
            }
        ...
        /**
         * @brief 「일문 도전」 / 「일문 재도전」 한 장.
         *
         * 진행도·보상·받기 버튼은 전부 감춘다 - 귀문에는 셀 숫자도 받을 것도
         * 없고, 이 카드가 그 순간 말해야 하는 것은 문장 하나다.
         */
        private void PaintTrial()
        {
            trialShown = true;

            // 카드 전체 탭이 퀘스트 화면을 여는 것을 막는다. 안 막으면 한 번의
            // 탭에 귀문과 퀘스트 화면이 함께 열린다
            if (cardScreenButton != null) cardScreenButton.Suppressed = true;

            Show(true);

            if (actionLabel != null)
            {
                actionLabel.text = PromotionTrialCatalog.EntranceTag;
                actionLabel.color = readyColor;
            }

            if (detailLabel != null)
            {
                detailLabel.text = PromotionTrialCatalog.GateName(fight.PendingTrialGate)
                                   + (fight.TrialAttempted
                                        ? PromotionTrialCatalog.RetrySuffix
                                        : PromotionTrialCatalog.ChallengeSuffix);
                detailLabel.color = normalColor;
            }

            if (rewardRoot != null && rewardRoot.activeSelf) rewardRoot.SetActive(false);
            if (claimRoot != null && claimRoot.activeSelf) claimRoot.SetActive(false);
            if (arrowLabel != null) arrowLabel.alpha = 1f;
        }

        /**
         * @brief 카드 전체 탭. **귀문 대기 중에만** 이쪽이 일한다.
         *
         * 평소의 탭(퀘스트 화면 열기)은 같은 버튼에 붙은 `HudScreenButton`이
         * 처리한다. 둘 다 리스너로 붙어 있고 서로를 모르므로, 각자 자기가
         * 일할 상태가 아니면 즉시 돌아간다 - 그 두 조건이 배타라 한 번의
         * 탭에 정확히 하나만 일어난다.
         */
        private void OnCardClicked()
        {
            ...
            // **문을 열기 전에** 이 프레임의 화면 열기를 막는다. 여는 순간
            // `BossFight.Changed`가 같은 콜스택에서 카드를 다시 그리고, 그때
            // `Suppressed`가 풀려 뒤이어 도는 `HudScreenButton`이 퀘스트 화면을
            // 연다 - 실기에서 귀문과 퀘스트가 함께 뜬 화면이 그것이었다
            if (cardScreenButton != null) cardScreenButton.SuppressThisFrame();
