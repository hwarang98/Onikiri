...
| 3 | ~~**9:19.5 이상에서 배경 위 하늘 여백**~~ — **4단계에서 해소.** `Sky`를 밴드 앵커에서 분리해 카메라 전체에 타일링하므로 클리어 색 의존이 없어짐 (`ONIKIRI_Step4_Report.md` §2.3) | ~~연출 확장 시~~ |
| 4 | **패럴랙스 스크롤 없음** — 배치만 함 (3단계 범위) | 연출 확장 시 |
| 5 | ~~**`Gras` 레이어가 캐릭터 뒤에 그려짐**~~ — **4단계에서 해소.** `SortingOrders.GroundCover`(70)로 캐릭터 앞으로 이동 (`ONIKIRI_Step4_Report.md` §2.2) | ~~선택~~ |
