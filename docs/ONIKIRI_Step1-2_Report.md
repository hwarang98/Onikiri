...
> ⚠️ **정정 (2026-08-04)** — 본 문서 §7-6의 "Spring Forest가 가장 잘 맞음" 권고는 **틀렸습니다.**
> 384×216이 밴드 높이와 맞는다는 치수만 보고 낸 판단이었고, Spring Forest에는 설 수 있는
> 지면선이 없습니다(앞 레이어가 덤불). 사양서 §6이 지정한 **Tiny Pixel Japan**을 사용합니다.
> 근거는 `ONIKIRI_Step3_Report.md` §4 참조.
>
> 같은 문서 §7-5의 보스 스케일 항목과 §2 참격 VFX 측정치(105px)도 3단계 보고서에서
> 재측정·정정되었습니다 (프레임당 실측 42px).
